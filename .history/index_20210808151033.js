    ...
    client.onMessage((message) => {
      //  if(message.body === 'oi'){
			...
			if(command === '/music')
			{
				...
				call2 = async (video_music)=>{
					console.log(`oh, eu toh funcionando... \n`)
					await client.sendVoiceBase64(message.from,video_music)
					.then((result) => {
				...
				calli = async (name_music_path)=>{
					...
					await client.sendVoice(message.from,name_music_path,name_music_path,'vai porra')
					.then((result) => {
						console.log('enviando una musica')
						console.log('Result: ', result); //return object success
					  })
