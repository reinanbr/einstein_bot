...
import { get_music_yt } from '../music_dl'
const gm = get_music_yt
    ...
    conn.on('chat-update', chatUpdate => {
        ...
        if (chatUpdate.messages && chatUpdate.count) {
            ...
            if(command == '/music')
			{
                ...
                conn.sendMessage(idAuthor, `musica ${music} requerida`, MessageType.text)
                
                const calli = (name_music) =>{
                    conn.sendMessage(
                        idAuthor, 
                        fs.readFileSync(name_music), // load a gif and send it
                        MessageType.audio, 
                        { mimetype: Mimetype.mp4Audio, caption: "hello!" }
                    )
                }
                gm(music,calli)
        ...
        } else console.log ('') // see updates (can be archived, pinned etc.)
    })
